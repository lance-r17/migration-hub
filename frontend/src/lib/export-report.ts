...
export async function exportProjectDetailsReport() {
  ...
  try {
    ...
    const baseHeaders = [
      ...
      'GBI Champion Name',
      'GBI Champion Email',
      'GBI Champion Delegate Name',
      'GBI Champion Delegate Email',
      'RTO',
      'RPO',
      '3-AZ Readiness',
    ...
    for (const project of projects) {
      ...
      const row: Record<string, string | number> = {
        ...
        'GBI Champion Name': gr?.gbiChampion?.name ?? '',
        'GBI Champion Email': gr?.gbiChampion?.email ?? '',
        'GBI Champion Delegate Name': gr?.gbiChampionDelegate?.name ?? '',
        'GBI Champion Delegate Email': gr?.gbiChampionDelegate?.email ?? '',
        'RTO': av?.rto ?? '',
        'RPO': av?.rpo ?? '',
