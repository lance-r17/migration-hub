...
export function DownstreamDependenciesDrawer({ open, onOpenChange, data, onSave }: Props) {
  const [draft, setDraft] = useState<DependencyEntry[]>([])

  useEffect(() => {
    if (open) setDraft(data?.downstream ?? [])
  }, [open, data])

  function handleSave() {
    onSave({ ...data, upstream: data?.upstream ?? [], downstream: draft })
    onOpenChange(false)
  }

  return (
    <SectionEditDrawer open={open} onOpenChange={onOpenChange} title="Edit Downstream Dependencies" onSave={handleSave}>
